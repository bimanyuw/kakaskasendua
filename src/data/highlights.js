export const highlights = [
  {
    href: "#journey",
    title: "Journey Map",
    desc: "Jelajahi desa melalui peta interaktif. Setiap titik menyimpan cerita, foto, dan kisah lokal yang autentik.",
    svgPath:
      "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z",
  },
  {
    href: "#wisata",
    title: "Wisata & Aktivitas",
    desc: "Trekking kawah, cooking class, kerajinan bambu, dan sewa baju Kawasaran — pengalaman yang tak terlupakan.",
    svgPath:
      "M13.5 5.5c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zM9.8 8.9L7 23h2.1l1.8-8 2.1 2v6h2v-7.5l-2.1-2 .6-3C14.8 12 16.8 13 19 13v-2c-1.9 0-3.5-1-4.3-2.4l-1-1.6c-.4-.6-1-1-1.7-1-.3 0-.5.1-.8.1L6 8.3V13h2V9.6l1.8-.7",
  },
  {
    href: "#umkm",
    title: "UMKM & Produk Lokal",
    desc: "Bunga potong, tanaman hias, kuliner khas, dan kerajinan lokal dari tangan masyarakat Kakaskasen Dua.",
    svgPath:
      "M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 3c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm7 13H5v-.23c0-.62.28-1.2.76-1.58C7.47 15.82 9.64 15 12 15s4.53.82 6.24 2.19c.48.38.76.97.76 1.58V19z",
  },
  {
    href: "#gunung",
    title: "Info Gunung & Keamanan",
    desc: "Status aktivitas Gunung Lokon dan Mahawu, arti level peringatan, dan panduan keselamatan wisatawan.",
    svgPath:
      "M14 6l-1-2H5v17h2v-7h5l1 2h7V6h-6zm4 8h-4l-1-2H7V6h5l1 2h5v6z",
  },
];